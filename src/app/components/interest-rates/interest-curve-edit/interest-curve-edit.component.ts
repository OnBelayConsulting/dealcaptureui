...
export class InterestCurveEditComponent {
  ...
  ngOnInit(): void {
    if (this.interestCurveId()) {
      let subscriber = this.interestIndexService.findInterestCurveById(this.interestCurveId()!).subscribe( {
        next: interestCurveSnapshot => {
          if (interestCurveSnapshot) {
            ...
            this.modifiedSnapshot.indexId = {
              id: interestCurveSnapshot.indexId?.id,
              code : interestCurveSnapshot.indexId?.code
            }
        ...
        error: err => {
          this.router.navigate(['interestrates','curves', 'list']);
        }
      });
  ...
  onDelete() {
    if (this.modifiedSnapshot.entityId?.id) {
      let snapshot : InterestCurveSnapshot = {
        entityState : 'DELETE',
        indexId : {
      ...
      let subscription = this.interestIndexService.saveInterestCurve(snapshot).subscribe({
        next: (data) => {this.transactionResult = data},
        error: (error: Error) => {console.log(error.message)}
      });
