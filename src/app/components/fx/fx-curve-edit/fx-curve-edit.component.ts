...
import {FxCurveSnapshot} from '../model/fx.model';
import {FxIndexQuickSearchComponent} from '../fx-index-quick-search/fx-index-quick-search.component';
import {PriceCurveSnapshot} from '../../pricing/model/price.model';

@Component({
  selector: 'app-fx-curve-edit',
  imports: [
    ReactiveFormsModule,
    FxIndexQuickSearchComponent
  ],
  templateUrl: './fx-curve-edit.component.html',
  styleUrl: './fx-curve-edit.component.scss'
})
export class FxCurveEditComponent {
  router = inject(Router);
  fxIndexService = inject(FxIndexService);
  destroyRef = inject(DestroyRef);

  fxCurveId = input<number | undefined>(undefined);
  ...
  private modifiedSnapshot: FxCurveSnapshot = {
    entityState: 'NEW',
    entityId: {
      id: undefined
    },
  ...
  ngOnInit(): void {
    if (this.fxCurveId()) {
      let subscriber = this.fxIndexService.findFxCurveById(this.fxCurveId()!).subscribe( {
        next: fxCurveSnapshot => {
          if (fxCurveSnapshot) {
            this.modifiedSnapshot.entityState = 'UNMODIFIED';
            this.modifiedSnapshot.version = fxCurveSnapshot.version;
            this.modifiedSnapshot.entityId = {
              id: fxCurveSnapshot.entityId!.id
            };
            this.modifiedSnapshot.indexId = {
              id: fxCurveSnapshot.indexId?.id,
              code : fxCurveSnapshot.indexId?.code
            }

            this.myForm.controls.indexName.setValue(fxCurveSnapshot.indexId.code);

            this.myForm.controls.curveDate.setValue(fxCurveSnapshot.detail?.curveDate);

            this.myForm.controls.observedDateTime.setValue(fxCurveSnapshot.detail!.observedDateTime);
            this.myForm.controls.frequency.setValue(fxCurveSnapshot.detail!.frequencyCodeValue);

          } else {
            this.router.navigate(['fx','curves', 'list']);
  ...
  private submitPrice(wasModified: boolean) {
    ...
    let subscription = this.fxIndexService.saveFxCurve(this.modifiedSnapshot!).subscribe({
  ...
  onDelete() {
    if (this.modifiedSnapshot.entityId?.id) {
      ...
      let subscription = this.fxIndexService.saveFxCurve(snapshot).subscribe({
        next: (data) => {this.transactionResult = data},
        error: (error: Error) => {console.log(error.message)}
      });
