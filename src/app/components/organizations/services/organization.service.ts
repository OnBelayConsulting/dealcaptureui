...
import {
  ...
  OrganizationRoleSummaryCollection,
  OrganizationSnapshot,
  OrganizationSnapshotCollection
...
export class OrganizationService {
  ...
  findOrganizations(searchCriteria: string, orgRoleType: string, start: number, limit:number): Observable<OrganizationSnapshotCollection> {
    const options = searchCriteria ?
      { params: new HttpParams().set('query', searchCriteria).set('orgRoleType', orgRoleType).set('start', start).set('limit', limit) } : {};
