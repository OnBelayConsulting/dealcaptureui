...
export class PriceIndexService {
    ...
    return this.http.post<TransactionResult>(this.apiUrl, priceIndex)
      .pipe(
        catchError( (error: HttpErrorResponse) => {
          console.log(error);
          if (error.status === 403)
            this.errorService.showError("Saving PriceIndex forbidden", '403', 'User does not have permission');
          else
    ...
    return this.http.post<TransactionResult>(this.apiUrl + '/curves', priceCurve)
      .pipe(
        catchError( (error: HttpErrorResponse) => {
          console.log(error);
          if (error.status === 403)
            this.errorService.showError("Saving Price curve forbidden", '403', 'User does not have permission');
          else
            this.errorService.showError("Saving price curve Failed on", error.error.errorCode, error.message);
          return throwError( () => new Error("Network error occurred."))
        })
      );
  }
  ...
  findPriceIndexEntityIds(searchCriteria: string, start: number, limit: number): Observable<EntityListItemCollection> {
    const options = searchCriteria ?
      { params: new HttpParams().set('query', searchCriteria).set('start', start).set('limit', limit) } : {};
